...
async function addMember(sh, email, name, date) {
    await sh.spreadsheets.values.append({
          ...
          requestBody: { values: [[email, name, date, 'Active']] }
    });
}
...
module.exports = async function handler(req, res) {
    ...
    try {
          if (event.type === 'customer.subscription.created') {
                  ...
                  if (!email) {
                            console.log('[sub.created] No email - skipping (test event)');
            ...
            const ex = await getMember(sh, email);
                  ...
                  if (!cust.email) {
                            console.log('[sub.deleted] No email - skipping (test event)');
                            return res.status(200).json({ received: true });
                  }
